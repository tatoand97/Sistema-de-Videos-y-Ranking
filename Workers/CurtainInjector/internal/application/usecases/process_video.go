...
func (uc *ProcessVideoUseCase) Execute(filename string) error {
	...
	processedData, err := uc.processingService.InjectCurtains(inputData, "/app/ANB_Curtain_IN.mp4", "/app/ANB_Curtain_OUT.mp4")
	if err != nil {
		uc.videoRepo.UpdateStatus(video.ID, domain.StatusFailed)
		return fmt.Errorf("failed to process video: %w", err)
	}

	outputFilename := fmt.Sprintf("curtained_%s", filename)
