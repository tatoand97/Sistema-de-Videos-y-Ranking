...
func TestConfigurationSecurity_Integration(t *testing.T) {
	...
	t.Setenv("TEST_AWS_REGION", "us-east-1")
	t.Setenv("TEST_AWS_ACCESS_KEY_ID", "testaccess")
	t.Setenv("TEST_AWS_SECRET_ACCESS_KEY", "testsecret123")
	...
	// Test S3 config
	region, _ := security.GetEnvWithValidation("TEST_AWS_REGION", "", true)
	accessKey, _ := security.GetEnvWithValidation("TEST_AWS_ACCESS_KEY_ID", "", true)
	secretKey, _ := security.GetEnvWithValidation("TEST_AWS_SECRET_ACCESS_KEY", "", true)

	err = security.ValidateS3Config(region, accessKey, secretKey)
	assert.NoError(t, err)
...
func TestEnvironmentValidation_Integration(t *testing.T) {
	...
	testEnv := map[string]string{
		...
		"AWS_ACCESS_KEY_ID":     "secure_access_key",
		"AWS_SECRET_ACCESS_KEY": "secure_secret_key_123",
		"S3_BUCKET_RAW":         "raw-videos",
		"S3_BUCKET_PROCESSED":   "processed-videos",
	...
	// S3 validation
	region, err1 := security.GetEnvWithValidation("AWS_REGION", "", true)
	accessKey, err2 := security.GetEnvWithValidation("AWS_ACCESS_KEY_ID", "", true)
	secretKey, err3 := security.GetEnvWithValidation("AWS_SECRET_ACCESS_KEY", "", true)

	if err1 != nil || err2 != nil || err3 != nil {
		errors = append(errors, err1, err2, err3)
	} else if err := security.ValidateS3Config(region, accessKey, secretKey); err != nil {
		errors = append(errors, err)
	}

	// Queue validation
	for _, queueEnv := range []string{"S3_BUCKET_RAW", "S3_BUCKET_PROCESSED", "STATE_QUEUE"} {
