...
func TestLoadConfig_WithEnvironmentVariables(t *testing.T) {
	...
	assert.Equal(t, "watermarking", config.QueueName)
...
func TestLoadConfig_WatermarkingSpecific(t *testing.T) {
	clearEnvVars()

	config := infrastructure.LoadConfig()

	// Test Watermarking specific defaults
	assert.Equal(t, 30, config.MaxSeconds) // Ignored in Watermarking but still configured
	assert.Equal(t, 3, config.MaxRetries)
	assert.Equal(t, 1000, config.QueueMaxLength)
}
...
func TestConfig_Structure(t *testing.T) {
	...
	assert.Equal(t, "us-east-1", config.S3Region)
	assert.Equal(t, "https://s3.us-east-1.amazonaws.com", config.S3Endpoint)
	assert.Equal(t, "test-key", config.S3AccessKey)
	...
	assert.Equal(t, "watermark-queue", config.QueueName)
